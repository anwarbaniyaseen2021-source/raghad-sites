(() => {
  ...
  const storage = (() => {
    try {
      const testKey = "__loving_homes_storage_test__";
  ...
  const syncPreferences = () => {
    const theme = normalizeTheme(root.dataset.theme) || "night";
    const lang = normalizeLang(root.lang) || "en";
  ...
  const translations = {
    en: {
      ...
      "faq.a6":
        ...
        "reviews.lead": "See what dog owners say about their experience at Loving Homes.",

        "reviews.section": "What Our Customers Say",

        "reviews.r1.name": "Sarah A.",
        "reviews.r1.body": "My dog stayed here for a week and loved it. The staff were friendly and professional.",

        "reviews.r2.name": "Omar K.",
        "reviews.r2.body": "Very clean facilities and a safe environment for dogs. I felt comfortable leaving my dog here.",

        "reviews.r3.name": "Lina M.",
        "reviews.r3.body": "The team clearly cares about animals. My dog returned home happy and relaxed.",

        "reviews.r4.name": "David R.",
        "reviews.r4.body": "Excellent service and communication. I will definitely book again.",
        "reviews.add.title": "Add Your Review",
        "reviews.form.name": "Your Name",
        "reviews.form.namePlaceholder": "Your name",
        "reviews.form.review": "Your Review",
        "reviews.form.reviewPlaceholder": "Write your experience...",
        "reviews.form.submit": "Submit Review",
        "nav.searchPlaceholder": "Search Page...",
    ...
    ar: {
      ...
      "reviews.lead": "اطلع على آراء أصحاب الكلاب حول تجربتهم في لوفينغ هومز.",
      "reviews.section": "ماذا يقول عملاؤنا",

      "reviews.r1.name": "سارة",
      "reviews.r1.body": "أقام كلبي هنا لمدة أسبوع وكان سعيداً جداً. الفريق ودود ومحترف.",

      "reviews.r2.name": "عمر",
      "reviews.r2.body": "المكان نظيف جداً وآمن للكلاب. شعرت بالراحة عندما تركت كلبي هنا.",

      "reviews.r3.name": "لينا",
      "reviews.r3.body": "الفريق يهتم بالحيوانات فعلاً. عاد كلبي سعيداً ومرتاحاً.",

      "reviews.r4.name": "ديفيد",
      "reviews.r4.body": "خدمة ممتازة وتواصل رائع. سأحجز مرة أخرى بالتأكيد.",
      "reviews.add.title": "أضف تقييمك",
      "reviews.form.name": "اسمك",
      "reviews.form.namePlaceholder": "اسمك",
      "reviews.form.review": "تقييمك",
      "reviews.form.reviewPlaceholder": "اكتب تجربتك...",
      "reviews.form.submit": "إرسال التقييم",
      "nav.searchPlaceholder": "ابحث عن الصفحة...",
      "nav.searchBtn": "اذهب",
    }
  };
  ...
  const applyLanguage = (lang, { persist = true } = {}) => {
    ...
    if (persist) {
      storage.setItem("lang", resolvedLang);
    }
    syncPreferences();
  };
  ...
  const setZoom = (value) => {
    ...
    storage.setItem("zoom", currentZoom.toString());
  };
  ...
  const storedZoom = parseFloat(storage.getItem("zoom"));
  ...
  const urlTheme = normalizeTheme(urlParams.get("theme"));
  const storedTheme = normalizeTheme(storage.getItem("theme"));
  const initialTheme = urlTheme || storedTheme || getAutoTheme();
  setTheme(initialTheme, { persist: !urlTheme && Boolean(storedTheme) });

  const urlLang = normalizeLang(urlParams.get("lang"));
  const storedLang = normalizeLang(storage.getItem("lang"));
  const initialLang = urlLang || storedLang || "en";
  applyLanguage(initialLang, { persist: !urlLang && Boolean(storedLang) });
...
const withPrefs = (pageUrl) => {
  try {
    const destination = new URL(pageUrl, window.location.href);
    const current = new URL(window.location.href);
    const theme = current.searchParams.get("theme");
    const lang = current.searchParams.get("lang");
    if (theme) destination.searchParams.set("theme", theme);
...
if (pageSearch && searchBtn) {
  ...
  const searchPage = () => {
    ...
    const map = pageMap[lang] || pageMap.en;
    let pageUrl = null;
    ...
    if (pageUrl) {
      window.location.href = withPrefs(pageUrl);
    } else {
      alert(lang === "ar" ? "لم يتم العثور على الصفحة" : "Page not found");
    }
  };

  searchBtn.addEventListener("click", searchPage);

  pageSearch.addEventListener("keyup", (e) => {
